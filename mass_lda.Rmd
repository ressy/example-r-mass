...
R package, using a simple idealized case and more of a geometric/vector
...
hist_grp(ld, "LD1", seq(-4, 4, 0.1))
```

This new number gives better separation between the groups than any of the 
individual measurements.  Most of the points in a given group are on one side or
the other of the origin along this new vector.

```{r}
# Does this work the way I think it might?
ld$Prediction <- c("Group1", "Group2")[(ld$LD1 >= 0) + 1]
confusion <- table(ld[, 2:3])
...
# Note that MASS provides predict.lda to make this magic work
p <- predict(lds)
...
confusion2 <- table(cbind.data.frame(observations$Group, p2))
confusion2_df <- as.data.frame(confusion2)
colnames(confusion2_df) <- c("Group", "Prediction", "Freq")
kable(confusion2_df)
```
